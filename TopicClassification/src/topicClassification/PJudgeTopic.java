...
public class PJudgeTopic {
	...
	public PJudgeTopic() throws IOException{
		...
		//日志文件
		logout=new FileWriter("run_sms_mageng_test.log",true);//打开run_sms_mageng_test.log文件
		fout=new FileWriter("run_sms_mageng_test.out",true);//打开run_sms_mageng_test.out文件
		//将stopword2.txt文件中的停用词加载到hashmap容器中
		Util.loadStopWordMap("D:\\JavaProject\\topicClassification\\src\\test\\stopword2.txt");	
		/*加载判断词典,即语料库
		#爱#
                     喜悦  0.236066 
                     笑  0.144262
                    ...
                    悲  0.052459 
		*/
		this.loadProbability("D:\\JavaProject\\topicClassification\\src\\test\\term_topic_probability_tall.txt");
		//加载排查词典  
		//我 好 高兴 啊 &喜悦
		//晚上 一起 吃饭 &吃饭
		//将文件43123_seg_topic.txt中的主题和词条归一化存入topic_map容器中
		//key值为:主题  value值为:主题下词条以及词条出现的频率
		check.spssTopicWord("D:\\JavaProject\\topicClassification\\src\\test\\43123_seg_topic.txt");
	}
	...
	public Pair<String,Double> judge(String sms) throws IOException
	{
		...
		for(int i=0;i<split.length;i++)//测试每个词的话题列表
		{
			...
			//以追加的方式写入run_sms_mageng_test.out文件中
	...
	{
		BufferedReader fin=new BufferedReader(new FileReader("D:\\JavaProject\\topicClassification\\src\\test\\in.txt"));
		File file = new File("D:\\JavaProject\\topicClassification\\src\\test\\out.txt");
		...
		while((strLine=fin.readLine())!=null)
		{
			if(strLine.compareTo("")!=0)
			{
				...
				Pair<String,Double> topic =tp.Pjudge(strLine);
...
/*
 读文件:
 term_topic_probability_tall.txt   
 43123_seg_topic.txt              
 stopword2.txt
 stopword_role.txt
 写文件:
 run_sms_mageng_test.log
 run_sms_mageng_test.out
 */
